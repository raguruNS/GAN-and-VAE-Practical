...
Generative Adversarial Network (GAN) using the Fashion-MNIST dataset.

Autoencoder (AE) and Variational Autoencoder (VAE) using the MNIST dataset.
...
Open GAN_VAE_.ipynb in Google Colab.

Select Runtime → Change runtime type.

Select a GPU such as T4 GPU if available.

Run the notebook cells from top to bottom.

Allow the GAN to complete 25 epochs.

Allow the VAE to complete 20 epochs.

Review the generated images, latent-space plots, and loss graphs.

The GAN generated images will be stored in gan_generated/.
